...
Download.prototype.start = function() {
	...
	this.options.onEnd = function(err, result) {
        ...
        var invalidFilePathError = (err) ? (((typeof err === 'object') ? err.toString() : err).indexOf('Invalid file path') == -1) : false;

        // If we encountered an error and it's not an "Invalid file path" error, we try to resume download "maxRetries" times
        if(err && invalidFilePathError && self._retryOptions._nbRetries < self._retryOptions.maxRetries) {
            self.setStatus(2);
